<script lang="ts">
    ...
    interface Stats {
        ...
        cacheHits: number;
        cacheHitRate: number;
        slowestAssetTime: number;
        totalSavedByCompression: number;
    }
    ...
    const getCategory = (res: PerformanceResourceTiming): string => {
        ...
        if (type === 'img' || type === 'image' || /\.(png|jpe?g|gif|svg|webp|avif|ico)$/.test(name))
            return 'image';
...
<div class="min-h-screen bg-black text-[#ededed] font-sans selection:bg-white/20">
    ...
    <nav class="border-b border-white/10 px-6 py-3 flex items-center justify-between bg-black/50 backdrop-blur-md sticky top-0 z-50">
        <div class="flex items-center gap-4">
            <div class="flex items-center gap-2">
                ...
                <span class="text-white font-medium text-sm">DevTools Pro Analyzer</span>
            </div>
        </div>
        <div class="flex items-center gap-4">
            <button
                    on:click={handleDownloadLog}
                    class="flex items-center gap-2 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/20 px-3 py-1.5 rounded-lg transition-colors text-xs font-bold"
            >
            ...
            {#if stats.heavyFiles > 0}
                <div class="flex items-center bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-1.5 gap-2 animate-pulse">
    ...
    <!-- Main Content -->
    <main class="p-6 max-w-[1400px] mx-auto space-y-8 relative z-10">
        <!-- Search and Refresh -->
        <div class="flex justify-between items-center gap-4">
            <div class="relative w-full max-w-xl">
                ...
                />
                <svg class="absolute left-3 top-2.5 text-gray-500" width="16" height="16" viewBox="0 0 24 24"
            ...
            <button
                    ...
                    class="bg-white text-black cursor-pointer px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors shrink-0"
            >
                Refresh Scan
            </button>
        </div>
            ...
            <div class="lg:col-span-3 space-y-4">
                ...
                <div class="space-y-3">
                    {#each filteredAssets as asset (asset.id)}
                        <div
                                class={`group bg-white/[0.02] border rounded-xl p-4 hover:border-white/20 transition-all cursor-pointer relative overflow-hidden ${
        asset.isHeavy ? 'border-red-500/40 bg-red-500/[0.02]' : 'border-white/10'
                        ...
                        >
                            ...
                            <div class="absolute top-0 right-0 flex">
                                {#if asset.isCached}
                                    <div class="px-3 py-1 bg-green-500/20 text-green-400 border-b border-l border-green-500/20 text-[9px] font-black uppercase tracking-tighter">
                                        Cached
                                    </div>
                                {/if}
                                {#if asset.isHeavy}
                                    <div class="px-3 py-1 bg-red-500 text-white text-[9px] font-black uppercase tracking-tighter">
                                        Heavy Resource
                                    </div>
                                {/if}
                            </div>
                                ...
                                <div class="flex items-center gap-4 min-w-0">
                                    <div class="w-10 h-10 bg-white/5 rounded-lg flex items-center justify-center border border-white/5 shrink-0">
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none"
                                             ...
                                             class={getCategoryIconColor(asset.category)}>
                                            <path d={getCategoryIcon(asset.category)}/>
                                        </svg>
                                    </div>
                                    <div class="min-w-0">
                                        <h3 class="font-medium text-[14px] group-hover:underline truncate">{asset.name}</h3>
                                        <p class="text-xs text-gray-500 font-mono truncate max-w-lg">{asset.path}</p>
                                    </div>
                                </div>
                                <div class="flex flex-col items-end gap-1.5 shrink-0 mr-12">
             <span class="text-[10px] px-2 py-0.5 rounded-full border border-white/10 bg-white/5 font-bold uppercase text-gray-400">
          {asset.initiator}
             </span>
                                    <span class="text-[10px] text-gray-500 font-mono">{asset.protocol}</span>
                                </div>
                            </div>

                            <div class="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-gray-400">
                                <div class={`flex items-center gap-1.5 ${asset.isHeavy ? 'text-red-400 font-bold' : 'text-gray-300'}`}>
                                    <div class={`w-2 h-2 rounded-full ${asset.isHeavy ? 'bg-red-500 animate-pulse' : 'bg-blue-500'}`}></div>
                                    {asset.size.toFixed(1)} KB
                                </div>
                                <div class={`flex items-center gap-1.5 ${asset.duration > SLOW_THRESHOLD ? 'text-yellow-500' : ''}`}>
                                ...
                                {#if asset.savedByCompression > 0}
                                    <div class="flex items-center gap-1.5 text-green-400/80">
                    ...
                    {#if filteredAssets.length === 0}
                        <div class="text-center py-12 border border-dashed border-white/10 rounded-xl text-gray-500">
            ...
            <div class="space-y-6">
                <h2 class="text-sm font-semibold text-gray-400 uppercase tracking-widest">Dashboards & Metrics</h2>
                ...
                <div class="bg-white/[0.02] border border-white/10 rounded-xl p-5 space-y-6">
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs">
                            <span class="text-gray-400">Total Network Transfer</span>
                            <span class="text-white font-mono">{(stats.totalSize / 1024).toFixed(2)} MB</span>
                        </div>
                        <div class="flex justify-between text-xs">
                            <span class="text-gray-500">Uncompressed Size</span>
                            <span class="text-gray-400 font-mono">{(stats.totalDecodedSize / 1024).toFixed(2)} MB</span>
                        </div>
                        <div class="w-full h-1.5 bg-white/5 rounded-full overflow-hidden mt-1 relative">
                            <div class="absolute top-0 left-0 h-full bg-white/30"
                                 style="width: {Math.min((stats.totalDecodedSize / 10000) * 100, 100)}%"></div>
                            <div class="absolute top-0 left-0 h-full bg-blue-500 transition-all duration-500"
                                 style="width: {Math.min((stats.totalSize / 10000) * 100, 100)}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs">
                            ...
                            <span class="text-green-400 font-mono">{stats.cacheHitRate.toFixed(1)}%</span>
                        </div>
                        <div class="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                            <div class="h-full bg-green-500 transition-all duration-500"
                                 style="width: {stats.cacheHitRate}%"></div>
                        </div>
                        <p class="text-[10px] text-gray-500 text-right">{stats.cacheHits} / {stats.totalRequests}
                            cached</p>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                            <div class="h-full transition-all duration-500 {stats.avgLoadTime > 500 ? 'bg-yellow-500' : 'bg-blue-400'}"
                                 style="width: {Math.min((stats.avgLoadTime / 1000) * 100, 100)}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3 pt-4 border-t border-white/5">
                        <div class="bg-white/5 rounded-lg p-3">
                            <div class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Requests</div>
                            <div class="text-lg font-bold text-white">{stats.totalRequests}</div>
                        </div>
                        <div class="bg-white/5 rounded-lg p-3">
                            <div class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Images</div>
                            <div class="text-lg font-bold text-pink-400">{stats.images}</div>
                        </div>
                        <div class="bg-white/5 rounded-lg p-3">
                            <div class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Scripts</div>
                            <div class="text-lg font-bold text-yellow-400">{stats.scripts}</div>
                        </div>
                        <div class="bg-white/5 rounded-lg p-3">
                            <div class="text-[10px] text-gray-400 uppercase tracking-wider mb-1">Bandwidth Saved</div>
                            <div class="text-sm font-bold text-green-400 mt-1">{(stats.totalSavedByCompression / 1024).toFixed(1)}
                                MB
                            </div>
                        </div>
                    </div>
                ...
                <!-- Top 3 Slowest Panel -->
                <div class="bg-white/[0.02] border border-white/10 rounded-xl p-5 space-y-4">
                    <h3 class="text-xs font-bold text-gray-300 uppercase flex items-center gap-2">
                    ...
                    <div class="space-y-3">
                        {#each topSlowestAssets as asset, i (i)}
                            <div class="flex flex-col gap-1">
                                <div class="flex justify-between text-[11px]">
                                    <span class="text-gray-400 truncate pr-2" title={asset.name}>{asset.name}</span>
                                    <span class="text-yellow-500 font-mono shrink-0">{asset.duration.toFixed(0)}
                                        ms</span>
                                </div>
                                <div class="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                                    <div class="h-full bg-yellow-500/50"
                                         style="width: {(asset.duration / stats.slowestAssetTime) * 100}%"></div>
                                </div>
                            </div>
                        {/each}
                        {#if topSlowestAssets.length === 0}
                            <p class="text-xs text-gray-600">Nenhum dado capturado ainda.</p>
                        {/if}
                    </div>
                </div>
                ...
                <!-- Alert or Success -->
                {#if stats.heavyFiles > 0}
                    <div class="bg-red-500/10 border border-red-500/20 rounded-xl p-5 border-l-4 border-l-red-500">
                        ...
                        <button
                                on:click={() => (activeFilter = 'heavy')}
                                class="text-xs text-white underline decoration-white/30 hover:decoration-white transition-all"
                        >
                            Filtrar arquivos pesados
                        </button>
                    </div>
                {:else}
                    <div class="bg-green-500/10 border border-green-500/20 rounded-xl p-5 border-l-4 border-l-green-500">
                        <h4 class="text-sm font-bold text-green-500 mb-1 flex items-center gap-2">
                            ✅ System Optimized
                        </h4>
                        <p class="text-xs text-gray-400">Nenhum recurso crítico detectado passando dos limites de
                            segurança.</p>
                    </div>
                {/if}
            </div>
...
<style>
    ...
    :global(.custom-scrollbar-hide) {
        scrollbar-width: none;
        -ms-overflow-style: none;
    }

    :global(.custom-scrollbar-hide::-webkit-scrollbar) {
        display: none;
    }
</style>